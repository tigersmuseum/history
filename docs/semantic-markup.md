...
We'd like documents to be _writeable_ as well as readable, which is to say there shouldn't be any significant technical skills or knowledge needed to create them.
...
If the document is plain natural language text,
then potentially complex and difficult _natural language processing_ is needed to make a machine readable model from it.
